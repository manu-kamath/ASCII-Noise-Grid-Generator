...
export class NoiseGridComponent implements AfterViewInit, OnDestroy {
  ...
  exportHighResPng(): void {
    ...
    link.download = 'ascii-noise-grid-1920x1080.png';
  ...
  exportAsSvg(): void {
        ...
        for (let j = 0; j < cols; j++) {
            ...
            if (shapeImageData) {
                ...
                if (shapePixelInfo.alpha > 0) {
                    ...
                    shapeChars += `<text x="${x}" y="${y}" opacity="${opacity.toFixed(3)}">${char}</text>\n`;
                }
            }

            if (!isShape) {
                noiseChars += `<text x="${x}" y="${y}">${char}</text>\n`;
            }
        }
    }
    ...
    const backgroundRect = currentConfig.transparentBg
      ? ''
      : `<rect width="100%" height="100%" fill="${currentConfig.backgroundColor}" />`;
    
    const svgContent = `
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" font-family="monospace" font-size="${fSize}px">
  ${defs}
  ${backgroundRect}
